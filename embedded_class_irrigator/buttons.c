...
uint8_t pollButtonA() { // Button A = PB3 = PCINT3 = astar 14

}

uint8_t pollButtonC() { // Button C = PB0 = PCINT0 = astar 17
...
int SetUpButtonAction(IO_struct * button, int release, void(*callback)(void)) {
  ...
  //TOGGLE_BIT(*(&_red)->port, _red.pin); //This ended up set meaning this part is definitely entered
  if (button->pin == BUTTONA) {
    if (release) {
